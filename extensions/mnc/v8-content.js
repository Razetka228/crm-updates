...
// @namespace    https://mnc-lead-centre.ru/
// @version      3.1.107
// @description  Интерфейс crm_v8.html с живыми данными из таблицы #cr-grid-table.
// @author       Codex
// @match        https://mnc-lead-centre.ru/admin/domain/customer-request/index*
...
(() => {
  'use strict';
  try{console.log('[v8 МНЧ type-patch 3.1.108] patchModerationCardInPlace патчит .tag-fixed (тип) + .call-pill (плашка «На прозвоне») — обновляются на главной in-place без перезагрузки');}catch(_){}
  try{console.log('[v8 МНЧ bulk-source 3.1.107] «Поиск номера»: строки «N партнер» / «Город (описание)» запоминаются по номеру → при Создать/Добавить заявку источник передаётся в новую вкладку');}catch(_){}
  ...
  const BULK_CALL_SIP_KEY = 'tm_bulk_call_sip_v1';                  // МНЧ: карта номер→sip для звонка (по «Источник:»)
  ...
  // Суффикс sip_id для звонка по направлению (бт=002, мнч=003; kp/прочее — без). Host-based → переживает cp+sed.
  ...
  // МНЧ: sip звонка по заголовку «Источник:» в буфере. «МНЧ Старая АТС» → sip_id=001, остальные → 003.
  ...
  function parseBulkCallSipMap(text) {
    ...
    String(text || '').split(/\r?\n/).forEach((line) => {
      ...
      if (src) { curSip = /старая/i.test(src[1]) ? ';sip_id=001' : ';sip_id=003'; return; }
      const m = line.match(/\+?[78][\s\-()]*\d(?:[\s\-()]*\d){9}/);
      if (!m) return;
      const phone = normalizeBulkPhone(m[0]);
  ...
  function getBulkCallSipForPhone(phone) {
    ...
    return tmSipCallSuffix(); // нет источника → направленческий (мнч=003)
  }
