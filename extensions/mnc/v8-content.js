...
(() => {
  'use strict';
  try{console.log('[v8 МНЧ type-patch 3.1.104] patchModerationCardInPlace теперь патчит .tag-fixed — тип заявки (Повтор/Гарантия) обновляется на главной in-place без перезагрузки');}catch(_){}
  try{console.log('[v8 МНЧ bulk-source 3.1.105] «Поиск номера»: строки «N партнер» / «Город (описание)» запоминаются по номеру → при Создать/Добавить заявку источник передаётся в новую вкладку');}catch(_){}
