...
(() => {
  'use strict';
  try{console.log('[v8 KP type-patch 3.1.108] patchModerationCardInPlace патчит .tag-fixed (тип) + .call-pill (плашка «На прозвоне») — обновляются на главной in-place без перезагрузки');}catch(_){}
  try{console.log('[v8 KP bulk-source 3.1.107] «Поиск номера»: строки «N партнер» / «Город (описание)» запоминаются по номеру → при Создать/Добавить заявку источник передаётся в новую вкладку');}catch(_){}
